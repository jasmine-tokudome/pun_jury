...
function check2(message){
    ...
    if (sentence.original.length != 0 && sentence.reading.length != 0){
        for (let noun of sentence.nouns){
    ...
    if (sentence.original.length != 0 &&
        ...
        sentence.pronunciation.length != 0) {
            for (let noun of sentence.nouns) {
                ...
                const hit_pronunciation = (sentence.pronounciation.match(new RegExp(noun.pronunciation, "g")) ?? []).length;
                ...
                if (hit_original < Math.max(hit_reading, hit_pronounciation, hit_fuzzy, hit_short)){
                    return true;
                }
            }
        }
